...
# Run health check
/root/Trickster/venv/bin/python scripts/monitor_git_sync.py --check >> "$LOG_FILE" 2>&1
...
if [ "$COMMITS_AHEAD" -gt 5 ]; then
    ...
    /root/Trickster/venv/bin/python scripts/monitor_git_sync.py --sync >> "$LOG_FILE" 2>&1
fi
